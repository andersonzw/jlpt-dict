...
const LevelSelect: React.FC<LevelSelectProps> = ({ selected }) => {
  ...
  return (
    <div className="flex flex-row justify-evenly mb-8 gap-2 px-4">
      ...
      <button
        className={`level-btn ${
          selected === "n2" ? "selected" : null
        } grow max-w-[95px]`}
        onClick={() => nav("/jlpt/n2")}
      >
        <span className="hidden sm:inline">JLPT</span> N2
      </button>

      <button
        className={`level-btn ${
          selected === "n3" ? "selected" : null
        } grow max-w-[95px]`}
        onClick={() => nav("/jlpt/n3")}
      >
